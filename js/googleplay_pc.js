...
(function(){
	...
	window.addEventListener('load', function(){
		oVAR = setVariables({
			...
			uBooks1 : document.getElementById('shelf1').querySelector('.book'),
			uBooks2 : document.getElementById('shelf2').querySelector('.book'),
			dSeeMoreShelf1 : document.getElementById('shelf1').querySelector('div.more'),
			dSeeMoreShelf2 : document.getElementById('shelf2').querySelector('div.more')
		});
	...
	function setElementStyle(){
		oVAR.uGenrelayer.style.display = "none";
		oVAR.dOverlay.style.display = "none"
		oVAR.dSeeMoreShelf1.style.display = "block";
		oVAR.dShelf2.style.display = "block";

		oVAR.uBooks1.style.height = oDATA.sShelfHeight;
		oVAR.uBooks2.style.height = oDATA.sShelfHeight;
	}
	...
	function attachEvents(){
		oVAR.dSeeMoreShelf1.addEventListener('click', loadBooks, false);
		oVAR.dSeeMoreShelf2.addEventListener('click', loadBooks, false);
		oVAR.uSelectGenre.addEventListener('click', loadBooks, false);
	...
	function handleHeading(id){
		var dHeadShelf1 = oVAR.dShelf1.querySelector('.heading');
		var hTitleShelf1 = dHeadShelf1.querySelector('h1');
		var sSubTitleShelf1 = dHeadShelf1.querySelector('span');

		var dHeadShelf2 = oVAR.dShelf2.querySelector('.heading');
		...
		switch(id){
			case "shlef1_more" :
				...
				oVAR.dSeeMoreShelf1.style.display = "none";
				break;
			...
			default :
				...
				oVAR.dSeeMoreShelf2.style.display = "block";
				break;
			}
	}
	...
	function changeBookContents(id, className){
		...
		switch(id){
			...
			// 첫 번째 서재의 더보기 클릭
				...
				oVAR.dShelf2.style.display = "none";
				// 두 번째 서재는 보여주지 않는다
				break;
			...
			// 장르 클릭
				...
				if(oVAR.dShelf2.style.display = "none")
					oVAR.dShelf2.style.display = "block";
				break;
		}
	}
	...
	function setShelfOverflow(tag){
		switch(tag){
			...
			// 더보기 클릭 시 모든 책의 list를 볼 수 있게 높이값 auto로 설정
				oVAR.uBooks1.style.height = "auto";
				break;
	...
	function getBookList(url) {
		...
		request.onreadystatechange = function() {		
			if(request.readyState === 4 && request.status === 200) {
				...
				oVAR.uBooks1.innerHTML = makeBookElement(result);
				oVAR.uBooks2.innerHTML = makeBookElement(result);
		...
		oVAR.dOverlay.style.display = "block"
